	...
	PSTR szCmdLine, int iCmdShow)
{
	...
	wc.hbrBackground = (HBRUSH)CreateSolidBrush(COLORREF(RGB(255,230,200)));
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		ReleaseDC(hwnd, hdc);
		SetWindowPos(hwnd, NULL, 100, 100, 1280, 720, 0);
		return 0;

	case WM_KEYDOWN:
		if (game->keyPress(wParam))
		{
			if (wndMode)
			{
				if (game->getMode() == game->GAME1P)
					SetWindowPos(hwnd, NULL, 100, 100, SCREEN_WIDTH * PX_PER_BLOCK + 165, SCREEN_HEIGHT * PX_PER_BLOCK + 115, 0);
			}
			else
			{
				if (game->getMode() != game->GAME1P)
			...
			if (game->isExit())
			{
				PostMessageW(hwnd, WM_CLOSE, wParam, lParam);
				return 0;
			}
		}
		return 0;
